...
void CL_ParseTEnt (void)
{
	...
	case TE_BLOOD:			// bullet hitting flesh
		MSG_ReadPos (&net_message, pos);
		MSG_ReadDir (&net_message, dir);
		CL_BloodEffect (pos, dir, 0xe8, 60);
		//CL_ParticleEffect (pos, dir, 0xe8, 60);
		r = 25 + (rand()%75);
		AddStain(pos, (20 + (r / 10)), (205 + (r / 2)), 0, 0, (205 + (r / 2)), stain_modulate);
//		AddStain(pos, (20+(rand()%10)), (179+(rand()%38)+(rand()%38)), 0, 0, (175+(rand()%100)), stain_modulate); //majstain
		break;
	...
	case TE_SHOTGUN:			// bullet hitting wall
		...
		CL_SmokeAndFlash(pos);
//		r = 10 + (rand()%50);
//		AddStain(pos, 5, r, r, r, (r * 4), stain_modulate);
		r = (79+(rand()%15)+(rand()%15));
		AddStain(pos, 5, r, r, r, (175+(rand()%100)), stain_modulate); //majstain
		break;
	...
	case TE_BLASTER:			// blaster hitting wall
		...
		AddStain(pos, 5, 50, 50, 0, 500, stain_subtract);
	...
	case TE_GRENADE_EXPLOSION_WATER:
		...
		r = ((rand()%30)+(rand()%30));
		AddStain(pos, 45, r, r, r, (175+(rand()%100)), stain_modulate); //majstain
...
//		cnt = MSG_ReadByte (&net_message);
	...
	case TE_MOREBLOOD:
		MSG_ReadPos (&net_message, pos);
		MSG_ReadDir (&net_message, dir);
		CL_BloodEffect (pos, dir, 0xe8, 250);
