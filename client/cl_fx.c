...
void CL_AddLightStyles (void)
{
	...
	for (i = 0, ls = cl_lightstyle; i < MAX_LIGHTSTYLES; i++, ls++)
	{
		if ( !i && Cvar_VariableValue("r_timebasedfx") )
		{
			time_t			ctime;
			struct tm		*ltime;
			char			hour_s[5];
...
void CL_LogoutEffect (vec3_t org, int type)
{
	...
	for (i=0 ; i<500 ; i++)
	{
		p = makePart (
			...
			crand()*20,						crand()*20,						crand()*20,
			0,								0,								-PARTICLE_GRAVITY,
			(((type == MZ_LOGIN)?0xd0:(type == MZ_LOGOUT)?0x40:0xe0)+ (rand()&7)),
			(((type == MZ_LOGIN)?0xd0:(type == MZ_LOGOUT)?0x40:0xe0)+ (rand()&7)),
...
void CL_ExplosionParticles (vec3_t org)
{
	...
	i = ((rand()%30)+(rand()%30));
	AddStain(org, 45, i, i, i, (175+(rand()%100)), stain_modulate); //majstain
}
...
void CL_RailTrail (vec3_t start, vec3_t end)
{
	...
	{	// new
		...
		for (i=0 ; i<len ; i++)
		{
			...
			for (j=0 ; j<3 ; j++)
			{
				p->org[j] = move[j] + dir[j]*3.0f*size;
				p->vel[j] = dir[j]*4.0f*size;
			}
			VectorAdd (move, vec2, move);
		}
	}
...
void CL_AddParticles (void)
{
	...
	for (p=active_particles ; p ; p=next)
	{
		...
		V_AddParticle (org, p->length, color, alpha, p->type);
		// PMM
		if (p->alphavel == INSTANT_PARTICLE)
		{
			p->alphavel = 0.0;
