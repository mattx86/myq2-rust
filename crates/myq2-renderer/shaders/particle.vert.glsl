...
void main() {
    ...
    float dist = length(a_Origin - pc.u_ViewOrigin);
    float distScale = dist < 20.0 ? 1.0 : 1.0 + dist * 0.004;
    float finalSize = clamp(a_Size * distScale, pc.u_MinSize, pc.u_MaxSize);

    // Billboard expansion
    vec3 right = pc.u_ViewRight * finalSize * 0.667;
    vec3 up = pc.u_ViewUp * finalSize * 0.667;
