...
void VID_Printf (int print_level, char *fmt, ...)
{
	...
	else if ( print_level == PRINT_INFO )
	{
		if (Cvar_VariableValue("r_verbose"))
			Com_Printf("%s", msg);
	}
...
============
*/
qboolean vid_ref_modified = false;
...
#if 0
	{
		cvar_t *gl_driver = Cvar_Get( "gl_driver", "opengl32", CVAR_ARCHIVE );
