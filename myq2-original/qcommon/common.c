...
void Qcommon_Init (int argc, char **argv)
{
...
#ifdef SEED_RANDOM
	srand ( time(NULL) ); // mattx86: seed_random
#endif

	z_chain.next = z_chain.prev = &z_chain;

	// mattx86: console_init
#ifdef CONSOLE_INIT_EARLY
	Con_Init ();
#endif
	...
	host_speeds = Cvar_Get ("host_speeds", "0", CVAR_ZERO);
	log_stats = Cvar_Get ("log_stats", "0", CVAR_ZERO);
	developer = Cvar_Get ("developer", "0", CVAR_ZERO);
	timescale = Cvar_Get ("timescale", "1", CVAR_ZERO);
	fixedtime = Cvar_Get ("fixedtime", "0", CVAR_ZERO);
	logfile_active = Cvar_Get ("logfile", "0", CVAR_ZERO);
	showtrace = Cvar_Get ("showtrace", "0", CVAR_ZERO);
	...
	s = va("Quake II v%4.2f (%s v%4.2f) %s %s [built: %s @ %s]", VERSION, DISTNAME, DISTVER, CPUSTRING, BUILDSTRING, __DATE__, __TIME__);
