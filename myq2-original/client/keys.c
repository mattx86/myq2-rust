...
====================
*/
extern int CharOffset (char *s, int charcount);
...
void Key_Console (int key)
{
	...
	if ( ( key == K_LEFTARROW ) || ( key == K_KP_LEFTARROW ) || ( ( key == 'h' ) && ( keydown[K_CTRL] ) ) )
	{
		...
		if (key_linepos > 1)
			key_linepos = CharOffset (key_lines[edit_line], key_linepos - 1);
		return;
	}
	...
	if ( key == K_RIGHTARROW )
	{
		...
		{
			if (strlen(key_lines[(edit_line + 31) & 31]) >= key_linepos) // mattx86: right arrow key fix, from pooy's orig.
				return;
		 ...
		 ( ( key == 'n' ) && keydown[K_CTRL] ) )
	{
		...
		{
			key_lines[edit_line][0] = ']';
			key_lines[edit_line][1] = 0; // mattx86: clear it, since the line from above will still be here
			key_linepos = 1;
		}
		else
		{
	...
	if (key == K_PGUP || key == K_KP_PGUP || key == K_MWHEELUP) // mattx86: mouse_wheel
	{
		con.display -= 3;
		return;
	}

	if (key == K_PGDN || key == K_KP_PGDN || key == K_MWHEELDOWN) // mattx86: mouse_wheel
	{
		con.display += 3;
	...
	if (key_linepos < MAXCMDLINE-1)
	{
		...
		if (key_insert)
		{
			...
			i = strlen(key_lines[edit_line]); // mattx86 / jitspoe: don't subtract 1 from this!!
...
void Key_Message (int key)
{
	...
	if ( key == K_ENTER || key == K_KP_ENTER )
	{
		if (chat_type == CT_PERSON)		//don't dbl quote
			Cbuf_AddText("say_person ");
		else if (chat_type == CT_TELL)	//don't dbl quote
			Cbuf_AddText("tell ");
...
void Key_Event (int key, qboolean down, unsigned time)
{
	...
	{/*
		...
		if (key != K_BACKSPACE 
			...
			&& key != K_MWHEELUP && key != K_MWHEELDOWN // mattx86: mouse_wheel
			&& key_repeats[key] > 1)
