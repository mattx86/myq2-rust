	...
	for (pnum = 0 ; pnum<frame->num_entities ; pnum++)
	{
			...
			s1->origin[2]!=s1->old_origin[2])
		{
			...
			if (s1->effects & EF_GIB)
				AddStain(s1->origin, (20 + (num / 10)), (num / 2), 0, 0, (num * 2), stain_modulate);
//				AddStain(s1->origin, (20+(rand()%10)), (25+(rand()%15)+(rand()%15)), 0 ,0, (125+(rand()%100)), stain_modulate); //majstain
			if (s1->effects & EF_GREENGIB)
				AddStain(s1->origin, (20 + (num / 10)), 0, (num / 2), 0, (num * 2), stain_modulate);
//				AddStain(s1->origin, (20+(rand()%10)), 0, (30+(rand()%20)+(rand()%20)), 0, (125+(rand()%100)), stain_modulate); //majstain
...
//============
		...
		{	// some bonus items auto-rotate
			...
			// mattx86: bobbing_items
#ifdef ENABLE_BOBBING_ITEMS
...
*/
#ifdef CENTERED_GUN
extern cvar_t *hand; // RIOT - Centered gun
#endif
...
#ifndef VISIBLE_GUN_WIDEANGLE // mattx86: gun_wideangle
	if (ps->fov > 90)
		return;
#endif
