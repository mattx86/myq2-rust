...
void Menu_DrawStatusBar( const char *string )
{
	if ( string )
	{
		...
		Draw_Fill( 0, VID_HEIGHT-8, VID_WIDTH, 8, 4, 1.0);
		Menu_DrawString( col*8, VID_HEIGHT - 8, string );
	}
	else
	{
		Draw_Fill( 0, VID_HEIGHT-8, VID_WIDTH, 8, 0, 1.0);
	}
}
...
void MenuList_Draw( menulist_s *l )
{
	...
	n = l->itemnames;
  	...
  	Draw_Fill( l->generic.x - 112 + l->generic.parent->x, l->generic.parent->y + l->generic.y + l->curvalue*10 + 10, 128, 10, 16, 1.0);
	while ( *n )
	{
