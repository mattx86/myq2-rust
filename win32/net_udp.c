...
#include "winsock2.h" // mattx86: majsock
#include "wsipx.h"
#include "../qcommon/qcommon.h"
#include "../qcommon/myq2opts.h" // mattx86: myq2opts.h

#ifndef SWAP_UDP_FOR_TCP // mattx86: udp_tcp
...
// mattx86: NET_IsLocalAdr
qboolean NET_IsLocalAdr(netadr_t a)
{
	// 127.xxx.xxx.xxx
	// 192.168.xxx.xxx
	// 172.16.xxx.xxx
...
#ifdef USE_WSAECONNRESET_FIX // mattx86: wsaeconnreset_fix
...
void NET_Init(void)
{
	...
	r = WSAStartup(wVersion, &winsockdata);

	if ( r || (r == 0 && winsockdata.wVersion != wVersion) )
	{
		Com_Printf("Failed to load the requested version of Winsock (v%i.%i).\n", LOBYTE(wVersion), HIBYTE(wVersion));
		Com_Printf("Trying v1.1..\n");

		if ( WSAStartup( MAKEWORD(1, 1), &winsockdata) != 0 )
			Com_Error(ERR_FATAL, "Winsock initialization failed.");
	}

	Com_Printf("Winsock v%i.%i initialized.\n", LOBYTE(winsockdata.wVersion), HIBYTE(winsockdata.wVersion));

	noudp = Cvar_Get("noudp", "0", CVAR_NOSET);
	noipx = Cvar_Get("noipx", "0", CVAR_NOSET);

	net_shownet = Cvar_Get("net_shownet", "0", CVAR_ZERO);
}
...
#if 0
		case WSAINVALIDPROCTABLE:	return "Invalid procedure table from service provider. (WSAINVALIDPROCTABLE)";
		case WSAINVALIDPROVIDER:	return "Invalid service provider version number. (WSAINVALIDPROVIDER)";
		case WSAPROVIDERFAILEDINIT:	return "Unable to initialize a service provider. (WSAPROVIDERFAILEDINIT)";
#endif
		case WSASYSCALLFAILURE:		return "System call failure. (WSASYSCALLFAILURE)";
		default:					return "NO ERROR";
	}
}
#endif // mattx86: udp_tcp