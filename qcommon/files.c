...
#include "qcommon.h"
#include "myq2opts.h" // mattx86: myq2opts.h
...
Loads the header and directory, adding the files at the beginning
of the list so they override previous pack/zip files.
=================
*/
...
//	checksum = Com_BlockChecksum ((void *)info, header.dirlen);
	...
	for (i = 0; i < MAX_FILES_IN_PACK; ++i)
	{
		...
		fread(&temp, sizeof(dzipheader_t), 1, packhandle);
...
#define DEFAULTZIP "zip"
int sortFiles_pak(char *arg1, char *arg2)
{
	char *arg1_1, *arg2_2;
	qboolean arg1base = false, arg2base = false;

	if (arg1)
	{
		arg1_1 = strchr(arg1, '/');
		if (arg1_1)
			arg1_1 = strchr(arg1_1+1, '/')+1;
		if (arg1_1)
			arg1base = !Q_strncasecmp (arg1_1, DEFAULTPAK, strlen(DEFAULTPAK));
	}
	if (arg2)
	{
		arg2_2 = strchr(arg2, '/');
		if (arg2_2)
			arg2_2 = strchr(arg2_2+1, '/')+1;
		if (arg2_2)
			arg2base = !Q_strncasecmp (arg2_2, DEFAULTPAK, strlen(DEFAULTPAK));
	}

	if (arg1base && !arg2base)
		return 0;
	if (arg2base && !arg1base)
		return 1;

	return Q_strcasecmp(arg1, arg2);
}

int sortFiles_zip(char *arg1, char *arg2)
{
	char *arg1_1, *arg2_2;
	qboolean arg1base = false, arg2base = false;
	...
	if (arg1)
	{
		...
		if (arg1_1)
			arg1_1 = strchr(arg1_1+1, '/')+1;
		if (arg1_1)
			arg1base = !Q_strncasecmp (arg1_1, DEFAULTZIP, strlen(DEFAULTZIP));
	}
	if (arg2)
	{
		arg2_2 = strchr(arg2, '/');
		if (arg2_2)
			arg2_2 = strchr(arg2_2+1, '/')+1;
		if (arg2_2)
			arg2base = !Q_strncasecmp (arg2_2, DEFAULTZIP, strlen(DEFAULTZIP));
	}
...
{
	char			findpaks[MAX_OSPATH], findzips[MAX_OSPATH];
	char			pakfile_list[2000][MAX_OSPATH];
	char			zipfile_list[2000][MAX_OSPATH];
	char			**pakfiles, **zipfiles;
	int				i, nfiles_pak, nfiles_zip;
	searchpath_t	*search;
//	pack_t			*pak;

	strcpy (fs_gamedir, dir);

	Com_sprintf(findpaks, sizeof(findpaks), "%s/%s.pak", dir, fs_paksearch->string);
	Com_sprintf(findzips, sizeof(findzips), "%s/%s.zip", dir, fs_paksearch->string);

	pakfiles = FS_ListFiles(findpaks, &nfiles_pak, 0, 0);
	zipfiles = FS_ListFiles(findzips, &nfiles_zip, 0, 0);
	...
	//
	// PAK file pak loading
	//
	if (pakfiles)
	{
		for (i = 0; i < nfiles_pak; i++)
		{
			if (! pakfiles[i])
				break;

			strcpy(pakfile_list[i], pakfiles[i]);
		}

		nfiles_pak = i;

//		qsort(pakfile_list, nfiles_pak, MAX_OSPATH, sortFiles_pak);

		for (i = 0; i < nfiles_pak; i++)
		{
			pack_t	*pak;

			pak = FS_LoadPackFile(&pakfile_list[i][0]);
			if (!pak)
				continue;
			search = Z_Malloc( sizeof(searchpath_t) );
			search->pack = pak;
	...
	if (pakfiles)
		FreeFileList(pakfiles, nfiles_pak);


	//
	// ZIP file pak loading
	//
	if (zipfiles)
	{
		for (i = 0; i < nfiles_zip; i++)
		{
			if (! zipfiles[i])
				break;

			strcpy(zipfile_list[i], zipfiles[i]);
		}

		nfiles_zip = i;

//		qsort(zipfile_list, nfiles_zip, MAX_OSPATH, sortFiles_zip);

		for (i = 0; i < nfiles_zip; i++)
		{
			pack_t	*pak;

			pak = FS_LoadZipFile(&zipfile_list[i][0]);
			if (!pak)
				continue;
			search = Z_Malloc( sizeof(searchpath_t) );
			search->pack = pak;
	...
	if (zipfiles)
		FreeFileList(zipfiles, nfiles_zip);

#if 0
	//
	// add any pak files in the format pak0.pak pak1.pak, ...
	//
	for (i = 0; i < MAX_PAKFILES; i++)	// mattx86: pakfile_amount
	{
	...
	//
	// add any pak files in the format pak0.zip pak1.zip, ...
	//
	for (i = 0; i < MAX_PAKFILES; i++)	// mattx86: pakfile_amount
	{
...
void FS_SetGamedir (char *dir)
{
	...
	//
#if 0 //ndef AUTO_CVAR // mattx86: auto_cvar
...
void FS_InitFilesystem (void)
{
	...
	//
	// paksearch <wildcard>
	// "pak*" for example, will load all PAK and ZIP files
	// that match it.. ie, "pakdmmaps.zip", "pak0.pak"..
	//
	fs_paksearch = Cvar_Get("paksearch", "pak*", CVAR_ARCHIVE);
