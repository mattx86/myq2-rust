	...
	if ( gl_dynamic->value )
	{
		...
		for ( surf = gl_lms.lightmap_surfaces[0]; surf != 0; surf = surf->lightmapchain )
		{
			...
			else
			{
				...
				if ( !LM_AllocBlock( smax, tmax, &surf->dlight_s, &surf->dlight_t ) )
				{
					VID_Printf( ERR_FATAL, "Consecutive calls to LM_AllocBlock(%d,%d) failed (dynamic)\n", smax, tmax );
				}
...
void R_RenderBrushPoly (msurface_t *fa)
{
	...
	{	
		...
		//GL_TexEnv (GL_MODULATE); // mattx86: this is already being modulated/over-brightened
...
void R_DrawAlphaSurfaces (void)
{
	...
	qglEnable (GL_BLEND);
	// mattx86: this modulates/over-brights EmitWaterPolys() / EmitWaterPolys_refl()
	...
	for (s=r_alpha_surfaces ; s ; s=s->texturechain)
	{
		...
		if (s->flags & SURF_DRAWTURB)
			...
			EmitWaterPolys (s);
#else
			EmitWaterPolys_refl(s); // MPO : only for transparent water
#endif
...
extern image_t		*detailtexture;  //ep::detail textures
extern image_t      *caustic_texture;  //ep::caustic textures
...
static void GL_RenderLightmappedPoly( msurface_t *surf )
{
	...
	if(r_caustics->value == 1)
		caustic_texture = GL_FindImage("fx/caustic.png", it_wall);
	else
		caustic_texture = GL_FindImage("fx/null.png", it_wall);

//ep::detail textures
	if(r_detailtexture->value == 1)
		detailtexture = GL_FindImage("fx/detail1.png", it_wall);  //eQ2 sensamilla - detailed textures
	else if(r_detailtexture->value == 2)
		detailtexture = GL_FindImage("fx/detail2.png", it_wall);  //eQ2 sensamilla - detailed textures
	else if(r_detailtexture->value == 3)
		detailtexture = GL_FindImage("fx/detail3.png", it_wall);  //eQ2 sensamilla - detailed textures
	else if(r_detailtexture->value == 4)
		detailtexture = GL_FindImage("fx/detail4.png", it_wall);  //eQ2 sensamilla - detailed textures
	else if(r_detailtexture->value == 5)
		detailtexture = GL_FindImage("fx/detail5.png", it_wall);  //eQ2 sensamilla - detailed textures
	else if(r_detailtexture->value == 6)
		detailtexture = GL_FindImage("fx/detail6.png", it_wall);  //eQ2 sensamilla - detailed textures
	else if(r_detailtexture->value == 7)
		detailtexture = GL_FindImage("fx/detail7.png", it_wall);  //eQ2 sensamilla - detailed textures
	else if(r_detailtexture->value == 8)
		detailtexture = GL_FindImage("fx/detail8.png", it_wall);  //eQ2 sensamilla - detailed textures
	else
		detailtexture = GL_FindImage("fx/null.png", it_wall);  //eQ2 sensamilla - detailed textures
//ep::detail textures


	if(image->has_alpha)
		qglEnable(GL_ALPHA_TEST); //ep ::alpha masking
...
dynamic:
	...
	if ( is_dynamic )
	{
		...
		if (!image->has_alpha && gl_state.num_tmu >= 4)
		{
			if ( !(surf->flags & SURF_UNDERWATER) )
				GL_MBind (GL_TEXTURE2, detailtexture->texnum);  //ep::detail textures
			else if ( surf->flags & SURF_UNDERWATER )
				GL_MBind (GL_TEXTURE2, caustic_texture->texnum);
		}

		if (surf->texinfo->flags & SURF_FLOWING || surf->flags & SURF_UNDERWATER)
		{
			scroll = -64 * ( (r_newrefdef.time / 40.0) - (int)(r_newrefdef.time / 40.0) );
			if(scroll == 0.0)
				scroll = -64.0;
		...
		for ( p = surf->polys; p; p = p->chain )
		{
			...
			for (i=0 ; i< nv; i++, v+= VERTEXSIZE)
			{
				...
				if (!image->has_alpha && gl_state.num_tmu >= 4)
				{
					// not under water; do detail texture
					if ( !(surf->flags & SURF_UNDERWATER) && r_detailtexture->value)
						qglMTexCoord2fSGIS( GL_TEXTURE2, v[3], v[4]);

					// under water; do caustics texture
					else if ( surf->flags & SURF_UNDERWATER && r_caustics->value)
						qglMTexCoord2fSGIS( GL_TEXTURE2, (v[3]+scroll/30), (v[4]+scroll/30)); // was '25', now '30'
				}
	...
	if(image->has_alpha)
	{
		qglDisable(GL_ALPHA_TEST);  //ep :: enable alpha testing
	}
}
...
void R_DrawInlineBModel (void)
{
	...
	if ( currententity->flags & RF_TRANSLUCENT )
	{
		...
		// mattx86: this modulates/over-brights R_RenderBrushPoly()
		if (!gl_config.mtexcombine || !r_overbrightbits->value)
		{
			GL_TexEnv (GL_MODULATE);
		}
...
void R_DrawWorld (void)
{
	...
	gl_state.currenttextures[0] = gl_state.currenttextures[1] = gl_state.currenttextures[2] = gl_state.currenttextures[3] = -1; //maj
...
void GL_CreateSurfaceLightmap (msurface_t *surf)
{
	...
	if ( !LM_AllocBlock( smax, tmax, &surf->light_s, &surf->light_t ) )
	{
		...
		if ( !LM_AllocBlock( smax, tmax, &surf->light_s, &surf->light_t ) )
		{
			VID_Printf( ERR_FATAL, "Consecutive calls to LM_AllocBlock(%d,%d) failed\n", smax, tmax );
		}
	}
