...
mleaf_t *Mod_PointInLeaf (vec3_t p, model_t *model)
{
	...
	if (!model || !model->nodes)
		VID_Printf (ERR_DROP, "Mod_PointInLeaf: bad model");
...
//			VID_Printf (ERR_DROP, "Bad surface extents");
	}
}
...
void Mod_LoadFaces (lump_t *l)
{
	...
	for ( surfnum=0 ; surfnum<count ; surfnum++, in++, out++)
	{
		...
		if (ti < 0 || ti >= loadmodel->numtexinfo)
			VID_Printf (ERR_DROP, "MOD_LoadBmodel: bad texinfo number");
...
{	
	...
	for ( i=0 ; i<count ; i++)
	{
		...
		if (j < 0 ||  j >= loadmodel->numsurfaces)
			VID_Printf (ERR_DROP, "Mod_ParseMarksurfaces: bad surface number");
...
void Mod_LoadBrushModel (model_t *mod, void *buffer)
{
	...
	if (loadmodel != mod_known)
		VID_Printf (ERR_DROP, "Loaded a brush model after the world");

	header = (dheader_t *)buffer;

	i = LittleLong (header->version);
	if (i != BSPVERSION)
		VID_Printf (ERR_DROP, "Mod_LoadBrushModel: %s has wrong version number (%i should be %i)", mod->name, i, BSPVERSION);
	...
	for (i=0 ; i<mod->numsubmodels ; i++)
	{
		...
		*starmod = *loadmodel;
		...
		if (starmod->firstnode >= loadmodel->numnodes)
			VID_Printf (ERR_DROP, "Inline model %i has bad firstnode", i);
...
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	...
	if (pheader->skinheight > MAX_LBM_HEIGHT)
		VID_Printf (ERR_DROP, "model %s has a skin taller than %d", mod->name,
				   MAX_LBM_HEIGHT);

	if (pheader->num_xyz <= 0)
		VID_Printf (ERR_DROP, "model %s has no vertices", mod->name);

	if (pheader->num_xyz > MAX_VERTS)
		VID_Printf (ERR_DROP, "model %s has too many vertices", mod->name);

	if (pheader->num_st <= 0)
		VID_Printf (ERR_DROP, "model %s has no st vertices", mod->name);

	if (pheader->num_tris <= 0)
		VID_Printf (ERR_DROP, "model %s has no triangles", mod->name);

	if (pheader->num_frames <= 0)
		VID_Printf (ERR_DROP, "model %s has no frames", mod->name);
...
void R_BeginRegistration (char *model)
{
	...
	flushmap = Cvar_Get ("flushmap", "0", CVAR_ZERO);
